...
if (!function_exists('_crawler_convert_youtube_embed_from_id_')) {
    function _crawler_convert_youtube_embed_from_id_($youtubeID = ''): string
    {
        ...
        $newHtml .= '<iframe loading="lazy" width="100%" height="445" src="https://www.youtube.com/embed/' . trim($youtubeID) . '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>';
        $newHtml .= '</div>';
        return trim($newHtml);
    }
}
if (!function_exists('_crawler_convert_image_src_from_url_')) {
    ...
    {
        $newImgSrc = '<img loading="lazy" class="news-posts-content-image" width="100%" src="' . trim($imgSrc) . '" title="' . trim($title) . '" alt="' . trim($alt) . '" />';
